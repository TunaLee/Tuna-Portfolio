...
const PROJECTS = [
  {
    ...
    period: "2025.08 — 현재",
    color: P.pk,
    tags: ["pyannote 3.1", "ECAPA-TDNN", "faster-whisper", "Qwen3-32B", "Neo4j", "FHIR R4B", "vLLM", "Label Studio"],
    ...
    highlights: [
      ...
      { title: "의료 특화 STT", desc: "faster-whisper large-v3-turbo + 의료 도메인 initial_prompt 엔지니어링 → Qwen3-32B LLM 후처리로 전문용어 자동 교정" },
      { title: "FHIR 자동생성", desc: "LLM 엔티티 추출 → Neo4j 지식그래프 → ICD-10/SNOMED CT/ATC 표준코드 매핑 → FHIR R4B Bundle. 환각률 25.6% → 정식 크로스맵 적용으로 개선" },
      { title: "데이터 품질검증", desc: "Label Studio 기반 55건 Ground Truth + pyannote DER 자동 평가 (speaker mapping + collar 적용)" },
    ...
    decision: "로컬 GPU 서버 선택 이유: 128GB 통합 메모리로 Qwen3-32B를 로컬에서 추론 가능. 의료기관 내부망에서 환자 데이터를 외부로 보내지 않고 처리할 수 있는 구조",
  },
  {
    id: "comduck",
    title: "싸덕 COMDUCK",
...
const TIMELINE = [
  ...
  { year: "2020", text: "클레온 입사 — 딥러닝 Worker, WebRTC, 미디어 서버 개발", type: "work" },
